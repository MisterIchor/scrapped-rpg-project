...
func add_container(container : DataContainer, display : PackedScene = load("res://src/GlobalUI/CharacterSheet/Display/DisplayContainerValue.tscn") as PackedScene) -> void:
	if feature_container.get_node_or_null(attribute.name):
		return
	
	var new_display : Control = display.instance()
	feature_container.add_child(display, true)
	display.set_value_tracker(attribute)
	display.connect("show_details", owner, "_on_show_details")
